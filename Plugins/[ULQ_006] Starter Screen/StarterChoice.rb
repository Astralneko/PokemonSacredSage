...
# Supports shiny chance (8x normal, 1/512 if default shiny chance is used), gender selection, and custom titles.
#
# -----------------------
# HOW TO USE
# -----------------------
...
#   title        : Custom text shown at top (default: "Choose your partner!")
...
class StarterChoice_Scene
  ...
  def pbStartScene(starters, level = 5, allow_cancel = true, title = nil, can_save = true, state_key = nil)
    ...
    title_text = title || _INTL("Choose your partner!")
  ...
  def createStarterDisplay(species, index)
    ...
    if saved_state
	  ...
	  # 8x chance for this mon to be shiny
      8.times do
        break if temp_pokemon.shiny?
        temp_pokemon.shiny = nil   # Make it recalculate shininess
        temp_pokemon.personalID = rand(2**16) | (rand(2**16) << 16)
	  end
  ...
  def getTypeColor(type)
    type_colors = {
      ...
      :CELESTIAL => Color.new(201, 135, 229),
      :GLITCH   => Color.new(128, 225, 184),
      :STELLAR   => Color.new(170, 219, 205)
    }
