#===============================================================================
	# Custom Weather Plugin - Glitch Weather
	# Horizontal coloured scan-line bars (cyan, blue, red, magenta) scatter across
	# the screen in a repeating cycle:
	#   • Fade in  (~30 frames)
	#   • Active   (~150 frames) — bars jump to new positions periodically
	#   • Fade out (~30 frames)
	#   • Silent   (~90 frames)
	# Total cycle: ~300 frames (~5 seconds at 60 fps)
#===============================================================================
...
class Spriteset_Map
	def glitch_make_bitmaps
		...
		# Each entry: [width, height, color]
		# Colors match the screenshot — cyan, electric blue, red, magenta/pink
		palettes = [
		...
		# Build a variety of bar widths and heights per colour
		...
		palettes.each do |col|
			widths.each do |w|
				heights.each do |h|
					bm = Bitmap.new(w, h)
					# Solid fill with slight left/right fade for a scanline feel
					fade = [w / 6, 12].min
	...
	def glitch_update_sprites
		...
		@glitch_sprites.each_with_index do |sprite, i|
			...
			# During active phase, bars periodically jump to a new random position
			if in_active
				jump_cd -= 1
				if jump_cd <= 0
					sprite.bitmap = @glitch_bitmaps[rand(@glitch_bitmaps.size)]
					...
					# Occasional horizontal offset glitch — bar shifts a few pixels right
					sprite.x += rand(30) - 5 if rand(8) == 0
				end
				@glitch_info[i] = [jump_cd, base_op]
			end
			
			# Individual opacity modulated by master
...
CustomWeather.register_weather(
	{ :id => :Glitch, :id_number => 21, :category => :None, :graphics => [],
		:tone_proc => proc { |strength|
			# Subtle desaturating digital tint — slight cyan push
